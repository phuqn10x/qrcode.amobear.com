...
function MobileNavigation(
  ...
  return (
    <>
      <div
        className={clsx(
          "fixed top-0 z-20 w-full bg-black h-14 flex md:hidden items-center justify-between px-6 _lg:px-12 break-words",
          props.className,
        )}
      >
        <Logo />
      ...
      <Link
        ...
        className={clsx(
          ...
          isActive
            ...
            : "text-zinc-600 dark:text-zinc-950",
        )}
      >
        {props.name}
...
function DesktopNavigation(
  ...
  return (
    <div
      className={clsx(
        " top-[40px] relative z-20 hidden md:flex break-words",
        props.className,
      )}
    >
      <ContainerHeader>
        <div className="h-[4.2rem] flex items-center justify-between p-4 bg-header rounded-[100px]">
          <Logo />
          <div className="flex items-center">
            {/* <nav>
              <ul className="flex text-[1rem] _font-medium text-zinc-800 dark:text-zinc-200 items-center">
                {props.links.map((item, index) => (
