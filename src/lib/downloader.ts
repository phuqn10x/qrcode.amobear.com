...
async function srcToImage(name: string, src: string) {
  ...
  if(src){
    ...
    const blob = await image.blob();
      createDownloadTask(URL.createObjectURL(blob), `QRcode_image`);
  }
  else{
    toast.error("Please generate the QR code !");
  }
...
const ApiFetcherQrcodeDownloaders: Record<string, Downloader> = withReport({
  jpg: ({ name, wrapper }) =>
    srcToImage(name, (wrapper.getElementsByTagName("img")[0]?.src) ? wrapper.getElementsByTagName("img")[0].src : "" ) ,
});
